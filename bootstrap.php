 ...
 * 
 * @author Guillaume
 * @version Lue depuis framework/changelog.json — ne pas la coder ici.
 *           Ce bloc annonçait 4.0.0-alpha.10 cinq versions après coup.
 */
...
function url(string $path): string {
    ...
    // Point de sortie unique : un module dont le préfixe public a été renommé
    // voit ici ses liens réécrits, sans qu'aucun de ses fichiers ne change.
    return BASE_URL . \Framework\Services\PublicPrefix::translate($path);
}
...
/**
 * La requête en cours est-elle un préchargement TurboNav (survol d'un lien,
 * pas un clic) ? TurboNav démarre un vrai fetch ~80ms après un simple survol
 * de la souris — toute route GET qui aurait un effet de bord (compteur,
 * marquage lu, etc.) doit vérifier ce helper et sauter cet effet si true,
 * sinon un simple passage de souris suffit à le déclencher.
 */
...
set_exception_handler(function ($exception) use ($config) {
    // Même règle qu'en base : on ne consigne que l'essentiel. Une page
    // introuvable et un jeton périmé sont des situations prévues, traitées
    // ci-dessous par une page dédiée ; les écrire ici ne faisait que remplir
    // php_error.log de lignes qu'on finit par ne plus lire.
    $prevu = $exception instanceof \Framework\Services\RouteNotFoundException
          || $exception instanceof \Framework\Security\CSRFException;
...
if (!IS_CLI) {
    ...
    // Filtrage géographique — juste après le pare-feu IP, dont il réutilise la
    // résolution d'adresse et la liste blanche. Inerte tant qu'aucun pays n'est
    // désigné dans le Centre de sécurité ; le coût se limite alors à la lecture
    // de deux réglages.
    $countryFirewall = new \Framework\Services\CountryFirewall($db);
    $GLOBALS['countryFirewall'] = $countryFirewall;
    (new \Framework\Middleware\CountryBlock($countryFirewall, $securityFirewallService))->handle();
...
// ─────────────────────────────────────────
// Préfixes publics des modules
// ─────────────────────────────────────────
// Doit précéder l'enregistrement des routes (index.php) : le routeur consulte
// cette carte au moment où chaque module déclare ses chemins.
$declaredPrefixes = [];
foreach ($moduleManager->discoverModules() as $moduleName => $moduleConfig) {
    $canonical = \Framework\Services\PublicPrefix::normalize(
        (string) ($moduleConfig['public']['prefix'] ?? '')
    );
    if ($canonical !== '') {
        $declaredPrefixes[$moduleName] = $canonical;
    }
}
\Framework\Services\PublicPrefix::load($db, $declaredPrefixes);
