...
$providerEmoji = ['openai' => '🟢', 'claude' => '🟣', 'anthropic' => '🟣', 'mistral' => '🟠', 'gemini' => '💎', 'groq' => '⚡', 'ollama' => '🦙', 'google' => '🔴'];
...
<?php else: ?>
    <?php $providerIndex = 0; foreach ($modelsByProvider as $provider => $list): $providerIndex++; $isOpen = $providerIndex === 1; ?>
        <div class="ui-card u-mt ai-accordion<?= $isOpen ? ' is-open' : '' ?>">
            <div class="ui-card-head ai-accordion-toggle" role="button" tabindex="0" aria-expanded="<?= $isOpen ? 'true' : 'false' ?>">
                <span class="ai-accordion-chevron">▸</span>
                <?= $providerEmoji[strtolower($provider)] ?? '🔌' ?> <?= $h(ucfirst($provider)) ?> <span class="ui-card-actions ui-badge"><?= count($list) ?> modèle(s)</span>
            </div>
            <div class="ui-card-body ai-accordion-panel" style="padding:0">
                <div style="overflow-x:auto">
                    <table class="ui-table">
                        <thead><tr><th>Modèle</th><th>Statut</th><th class="u-right">Actions</th></tr></thead>
                        <tbody>
                        <?php foreach ($list as $model): $id = (int)$model['id']; ?>
                            <tr>
                                <td>
    ...
    <style>
        .ai-accordion-toggle { cursor: pointer; user-select: none; display: flex; align-items: center; gap: 8px; }
        .ai-accordion-toggle:focus-visible { outline: 2px solid var(--accent); outline-offset: -2px; }
        .ai-accordion-chevron { display: inline-block; transition: transform .15s ease; font-size: 12px; color: var(--muted); }
        .ai-accordion.is-open .ai-accordion-chevron { transform: rotate(90deg); }
        .ai-accordion-panel { display: none; }
        .ai-accordion.is-open .ai-accordion-panel { display: block; }
    </style>
    <script>
        document.querySelectorAll('.ai-accordion-toggle').forEach(function (toggle) {
            var card = toggle.closest('.ai-accordion');
            function flip() {
