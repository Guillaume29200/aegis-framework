...
class SitemapService
{
    ...
    /**
     * URL absolue d'un chemin appartenant à un module.
     *
     * Le préfixe public d'un module étant renommable, un chemin canonique comme
     * « /forum/t/x » doit être traduit vers l'adresse en vigueur : sans cela le
     * sitemap annoncerait des URL qui ne répondent qu'en 301, ce que les moteurs
     * pénalisent. On ne peut pas employer u() ici : $baseUrl porte déjà BASE_URL,
     * qu'il rajouterait une seconde fois.
     */
    private function loc(string $baseUrl, string $path): string
    {
        return $baseUrl . \Framework\Services\PublicPrefix::translate($path);
    }
    ...
    private function collectUrls(string $baseUrl): array
    {
        ...
        if ($this->tableExists('forum_categories')) {
            $urls[] = ['loc' => $this->loc($baseUrl, '/forum'), 'lastmod' => $today, 'changefreq' => 'daily', 'priority' => '0.9'];
            ...
            try {
                ...
                foreach ($cats as $c) {
                    $urls[] = ['loc' => $this->loc($baseUrl, '/forum/c/' . $c['slug']), 'changefreq' => 'daily', 'priority' => '0.7'];
                }
            ...
            try {
                ...
                foreach ($topics as $t) {
                    $urls[] = [
                        'loc' => $this->loc($baseUrl, '/forum/t/' . $t['slug']),
        ...
        if ($this->tableExists('forum_pages')) {
            try {
                ...
                foreach ($pages as $p) {
                    $urls[] = ['loc' => $this->loc($baseUrl, '/forum/page/' . $p['slug']), 'changefreq' => 'monthly', 'priority' => '0.4'];
                }
            } catch (\Throwable $e) { /* ignore */ }
        }

        // Tournois (si tables présentes)
        if ($this->tableExists('tournaments')) {
            foreach (['/tournaments', '/tournaments/tournois', '/tournaments/gathers',
                      '/tournaments/joueurs', '/tournaments/equipes', '/tournaments/jeux'] as $p) {
                $urls[] = ['loc' => $this->loc($baseUrl, $p), 'changefreq' => 'daily', 'priority' => '0.8'];
            }
            try {
                $t = $this->db->query(
                    "SELECT slug, format, updated_at FROM tournaments
                     WHERE slug IS NOT NULL AND slug != '' ORDER BY created_at DESC LIMIT 5000"
                );
                foreach ($t as $row) {
                    $seg = ($row['format'] ?? '') === 'gather' ? '/g/' : '/t/';
                    $urls[] = [
                        'loc' => $this->loc($baseUrl, '/tournaments' . $seg . $row['slug']),
            ...
            if ($this->tableExists('tournaments_games')) {
                try {
                    foreach ($this->db->query("SELECT slug FROM tournaments_games WHERE is_active = 1 AND slug IS NOT NULL AND slug != ''") as $g) {
                        $urls[] = ['loc' => $this->loc($baseUrl, '/tournaments/game/' . $g['slug']), 'changefreq' => 'weekly', 'priority' => '0.5'];
                    }
                } catch (\Throwable $e) { /* ignore */ }
            }
            if ($this->tableExists('tournaments_blog_posts')) {
                try {
                    foreach ($this->db->query("SELECT slug, published_at FROM tournaments_blog_posts WHERE is_published = 1 AND slug IS NOT NULL AND slug != '' ORDER BY published_at DESC LIMIT 2000") as $b) {
                        $urls[] = [
                            'loc' => $this->loc($baseUrl, '/tournaments/blog/' . $b['slug']),
                            'lastmod' => !empty($b['published_at']) ? date('Y-m-d', strtotime((string)$b['published_at'])) : null,
