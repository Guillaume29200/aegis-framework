...
class AuthService
{
    ...
    public function login(string $identifier, string $password, bool $rememberMe = false, ?string $screenResolution = null): array
    {
        ...
        // Mot de passe bon, compte actif : les compteurs peuvent retomber.
        ...
        // SECOND FACTEUR
        // ========================================
        // Le point d'arrêt est ici, AVANT createUserSession() : tant que le
        // code n'est pas vérifié, aucune session authentifiée n'existe. Placer
        // le contrôle après aurait laissé une session valide pendant tout le
        // temps de la saisie — et une simple navigation vers une autre adresse
        // aurait suffi à contourner le second facteur.
        $twoFactor = new TwoFactorService($this->db);

        if ($twoFactor->requisPour($user)) {
            $envoi = $twoFactor->emettre($user);

            if (!$envoi['success']) {
                return ['success' => false, 'error' => $envoi['message']];
            }

            // On ne retient que l'identifiant : la session d'attente ne doit
            // porter aucun droit. `twofa_pending_at` borne la validité de
            // l'attente elle-même.
            $_SESSION['twofa_pending_user']  = (int) $user['id'];
            $_SESSION['twofa_pending_at']    = time();
            $_SESSION['twofa_pending_remember'] = $rememberMe;

            return [
                'success'         => false,
                'twofa_required'  => true,
                'redirect'        => $this->basePath() . '/auth/2fa',
                'error'           => $envoi['message'],
            ];
        }

        return $this->finaliserConnexion($user, $rememberMe, $screenResolution);
    }

    /**
     * Termine une connexion interrompue par le second facteur.
     *
     * N'est appelée qu'après vérification du code par TwoFactorService : elle
     * ne contrôle pas le mot de passe, qui l'a déjà été avant l'envoi du code.
     * L'appelant DOIT donc s'assurer que la vérification a réussi.
     *
     * @return array{success:bool, user?:array, redirect?:string, error?:string}
     */
    public function terminerAvecDeuxiemeFacteur(int $userId, bool $rememberMe = false): array
    {
        $user = $this->getUserById($userId);

        // Le compte a pu être suspendu entre l'envoi du code et sa saisie.
        if (!$user || ($user['status'] ?? '') !== 'active') {
            return ['success' => false, 'error' => 'Ce compte n\'est plus accessible.'];
        }

        return $this->finaliserConnexion($user, $rememberMe, null);
    }

    /**
     * Tout ce qui suit l'authentification réussie : session, traçage,
     * « se souvenir de moi », date de dernière connexion.
     *
     * Extrait de `login()` pour que le parcours avec second facteur emprunte
     * exactement le même chemin — deux copies auraient fini par diverger, et
     * c'est la moitié qu'on oublie de mettre à jour qui devient la faille.
     *
     * @param array<string,mixed> $user
     */
    private function finaliserConnexion(array $user, bool $rememberMe, ?string $screenResolution): array
    {
        ...
        if ($this->authTracker) {
            ...
            // Détection de connexion suspecte (changement pays/appareil/IP rapide).
            // Choix : ne jamais bloquer la connexion (trop de faux positifs — VPN,
            // 4G/WiFi, voyage légitime) ; on notifie l'utilisateur et on trace côté
            // audit pour que les admins puissent réagir si un compte est ciblé.
            $suspiciousReasons = $this->authTracker->detectSuspiciousLogin((int)$user['id']);
            if (!empty($suspiciousReasons)) {
                $messages = array_column($suspiciousReasons, 'message');
                $summary = implode(' — ', $messages);
                $isHighSeverity = in_array('high', array_column($suspiciousReasons, 'severity'), true);

                \Framework\Services\NotificationService::notify(
                    (int)$user['id'],
                    'security',
                    $isHighSeverity ? 'Connexion inhabituelle détectée' : 'Nouvelle connexion',
                    $summary . ". Si ce n'était pas vous, changez votre mot de passe immédiatement.",
                    '/member/sessions',
                    $isHighSeverity ? '⚠️' : '🔔'
                );
    ...
    public function logout(): void
    {
        ...
        // Détruire la session PHP — vider $_SESSION et expirer le cookie AVANT
        // session_destroy() (comme SessionManager::destroy()). Sans ça, une requête
        // concurrente (ex. préchargement TurboNav déclenché par un simple survol de
        // souris sur le lien de déconnexion) qui lit encore l'ancien cookie de session
        // peut se retrouver avec un état de session à moitié détruit, provoquant une
        // erreur générique lors de la reconnexion immédiate qui suit.
    ...
    public function resetPasswordWithToken(string $token, string $password, string $passwordConfirm): array
    {
        ...
        if ($password === '') {
            ...
            // Même politique qu'à l'inscription : réinitialiser ne doit pas
            // être une porte dérobée vers un mot de passe plus faible.
            $manquements = \Framework\Security\PasswordPolicy::depuisConfig()->verifier($password);
            if ($manquements !== []) {
                $errors['password'] = implode(' ', $manquements);
            }
        }
    ...
    private function recordUserSession(int $userId): void
    {
        ...
        // Appareil / navigateur / OS sont déduits du seul User-Agent : analyse de
        // chaîne, aucune requête réseau, donc aucun coût sur le temps de connexion.
        //
        // La géolocalisation est volontairement absente : elle interroge des
        // services externes (jusqu'à 3 providers × 5 s de timeout) et trackLogin()
        // la réalise déjà pour la table user_logins. La refaire ici doublerait
        // cette latence à chaque connexion.
        $deviceType = $browser = $os = null;
        try {
            $detector   = new \Framework\Services\DeviceDetector($userAgent);
            $deviceType = $detector->getDeviceType();
            $browser    = $detector->getBrowser();
            $os         = $detector->getOS();
        } catch (\Throwable $e) {
            // Le suivi de session ne doit jamais empêcher une connexion.
            error_log('recordUserSession detection failed: ' . $e->getMessage());
        }

        $stmt = $this->db->prepare("
            INSERT INTO user_sessions (user_id, session_id, ip_address, user_agent, device_type, browser, os, last_activity)
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
        ");
    ...
    private function validateRegistration(array $data): array
    {
        ...
        if (empty($data['email'])) {
            ...
            // Filtrage des domaines (adresses jetables). Éteint par défaut ;
            // ne s'applique qu'à l'inscription publique, jamais à la création
            // d'un compte par un administrateur.
            $refus = \Framework\Security\EmailDomainPolicy::depuisBase($this->db)->refus((string) $data['email']);
            if ($refus !== null) {
                $errors['email'] = $refus;
            }
        }
        
        // Password — la règle vient de config/security.php, pas d'ici.
        if (empty($data['password'])) {
            $errors['password'] = 'Le mot de passe est requis.';
        } else {
            $manquements = \Framework\Security\PasswordPolicy::depuisConfig()->verifier((string) $data['password']);
            if ($manquements !== []) {
                $errors['password'] = implode(' ', $manquements);
            }
        }
    ...
    /** Le sous-dossier d'installation, s'il y en a un (gère Windows et Linux). */
    private function basePath(): string
    {
        $scriptName = str_replace('\\', '/', dirname($_SERVER['SCRIPT_NAME'] ?? ''));

        return ($scriptName === '/' || $scriptName === '' || $scriptName === '.') ? '' : $scriptName;
    }
