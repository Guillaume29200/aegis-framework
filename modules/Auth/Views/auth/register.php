...
// Visuels propres à la page d'inscription, avec repli sur ceux de la page de
// connexion s'ils ne sont pas définis (puis sur les images par défaut).
$registerCoverImage = trim((string)($settings['register_cover_image'] ?? '')) ?: trim((string)($settings['login_cover_image'] ?? ''));
$registerLogoImage  = trim((string)($settings['register_logo_image'] ?? ''))  ?: trim((string)($settings['login_logo_image'] ?? ''));
$coverImageUrl = $registerCoverImage !== '' ? u($registerCoverImage) : u('/framework/assets/images/login-images/login-cover.svg');
$logoImageUrl  = $registerLogoImage  !== '' ? u($registerLogoImage)  : u('/framework/assets/images/logo.png');
$loginVisualBadge = trim((string)($settings['register_visual_badge'] ?? '')) ?: (trim((string)($settings['login_visual_badge'] ?? '')) ?: 'Rejoignez la communauté');
$loginVisualTitle = trim((string)($settings['register_visual_title'] ?? '')) ?: (trim((string)($settings['login_visual_title'] ?? '')) ?: $siteName);
$loginVisualText  = trim((string)($settings['register_visual_text'] ?? ''))  ?: (trim((string)($settings['login_visual_text'] ?? '')) ?: "Créez votre compte en quelques secondes et accédez à l'ensemble de vos services.");
        ...
        <section class="auth-panel">
            <div class="auth-card">
                ...
                <form id="registerForm" method="POST" action="<?= u('/auth/register') ?>" class="auth-form">
                    ...
                    <div class="f-grid2">
                        <?php $pwPolicy = \Framework\Security\PasswordPolicy::depuisConfig(); ?>
                        <div class="f-field"><label for="password">Mot de passe *</label><input type="password" id="password" name="password" required autocomplete="new-password" minlength="<?= $pwPolicy->longueurMinimale() ?>"><p class="f-hint">🔐 <?= htmlspecialchars($pwPolicy->enonce()) ?></p><div id="error-password" class="error-message"></div></div>
