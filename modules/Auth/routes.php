...
return function($router) {
    ...
    $router->group('/auth', function($router) {
        ...
        // Session active : keep-alive (modale d'inactivité) + fermeture des autres sessions.
        $router->get('/session/ping',            'Auth\\Controllers\\SessionController@ping');
        $router->post('/session/logout-others',  'Auth\\Controllers\\SessionController@logoutOthers');
        ...
        // Double authentification par e-mail. Accessible sans session
        // authentifiée : à ce stade le mot de passe est validé mais la session
        // n'est pas encore ouverte — c'est tout l'objet du second facteur.
        $router->get('/2fa',         'Auth\\Controllers\\AuthController@showTwoFactor');
        $router->post('/2fa/verify', 'Auth\\Controllers\\AuthController@verifyTwoFactor');
        $router->post('/2fa/resend', 'Auth\\Controllers\\AuthController@resendTwoFactor');
        $router->post('/2fa/recover', 'Auth\\Controllers\\AuthController@recoverTwoFactor');
    });
