...
class AdminController
{
    ...
    public function dashboard(): void
    {
        ...
        // Le tableau de bord ne parle plus seulement des utilisateurs : il dit
        // d'abord si l'installation va bien. Le service vit dans le framework,
        // et le gestionnaire de modules arrive par le conteneur global — cette
        // page appartient à Auth mais rend un écran qui le dépasse.
        $health   = ['status' => 'ok', 'ok' => 0, 'warn' => 0, 'error' => 0, 'issues' => [], 'updates' => [], 'env' => []];
        $activity = [];
        $security = ['failed_24h' => 0, 'success_24h' => 0, 'blocked' => 0, 'targets' => [], 'suspect' => false];
        $storage  = ['free' => null, 'total' => null, 'used_pct' => null, 'dirs' => [], 'unwritable' => []];
        $audience = ['state' => 'absent', 'pageviews' => 0, 'visitors' => 0, 'days' => []];
        try {
            $mm = $GLOBALS['moduleManager'] ?? null;
            if ($mm instanceof \Framework\ModuleManager\ModuleManager) {
                $svc      = new \Framework\Services\DashboardHealthService($this->db, $mm);
                $health   = $svc->snapshot();
                $activity = $svc->recentActivity(7);
                // Ces trois relevés coûtent une dizaine de millisecondes en
                // tout : ils sont lus à chaque affichage, sans mémorisation,
                // pour ne jamais montrer un chiffre de sécurité périmé.
                $security = $svc->security();
                $storage  = $svc->storage();
                $audience = $svc->audience();
            }
        } catch (\Throwable $e) {
            // Un indicateur en panne ne prive pas l'administrateur de sa page.
        }
    ...
    {
        // Auth est un module cœur et ne doit pas dépendre d'un module tiers
        // (GameNodePanel) pour fonctionner. tableExists() suffit tant que GNP
        // n'est pas installé (aucune table gnp_*), mais si la table existait un
        // jour sans que les classes GameNodePanel soient chargeables (module
        // tiers partiellement présent), on dégrade proprement plutôt que de
        // faire planter la suppression d'utilisateur avec une erreur fatale.
        if (!$this->tableExists('gnp_game_servers')) {
            return 0;
        }
        if (!class_exists(DedicatedServerService::class) || !class_exists(DockerRuntimeService::class) || !class_exists(FTPService::class) || !class_exists(SSHService::class)) {
            $this->logger->warning('Table gnp_game_servers presente mais classes GameNodePanel indisponibles — nettoyage GNP ignore', ['user_id' => $userId]);
            return 0;
        }
