...
class ModuleGeneratorService
{
    ...
    public function generate(array $in): array
    {
        ...
        try {
            ...
            file_put_contents($target . '/module.json',      $this->tplManifest($name, $display, $desc, $author, $category, $icon, $routeBase, $sections, $mega));
            ...
            file_put_contents($target . '/Controllers/AdminController.php', $this->tplController($name, $sections));
            ...
            // Vues
            file_put_contents($target . '/Views/admin/dashboard.php', $this->tplDashboardView($display, $icon, $prefix, $sections, $routeBase));
    ...
    private function tplManifest(string $name, string $display, string $desc, string $author, string $category, string $icon, string $routeBase, array $sections, bool $mega): string
    {
        ...
        $manifest = [
            ...
            'menu' => [$menuItem],
            'requires' => ['cms_version' => '>=4.0.0', 'php_version' => '>=8.1.0', 'modules' => ['Auth']],
        ];
...
PHP;
    ...
    private function tplController(string $name, array $sections): string
    {
        $methods = '';
        foreach ($sections as $slug => $s) {
...
JSON;
    ...
    private function tplDashboardView(string $display, string $icon, string $prefix, array $sections, string $routeBase): string
    {
        $links = '';
        foreach ($sections as $slug => $s) {
