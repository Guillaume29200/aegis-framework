...
class DiagnosticService
{
    ...
    public function repair(string $fix, string $target): array
    {
        switch ($fix) {
            ...
            case 'update':
                ...
                // L'indicateur de l'en-tête mémorise son compte quelques
                // minutes. Sans cette purge, il continuait d'annoncer une
                // mise à jour qui venait d'être appliquée — le diagnostic
                // disait vrai, l'en-tête mentait.
                $this->forgetUpdateBadge();
    ...
    /** Oublie le compte mémorisé par l'indicateur de l'en-tête. */
    private function forgetUpdateBadge(): void
    {
        try {
            (new \Framework\Services\ModuleUpdateService($this->db, $this->modules))->forget();
        } catch (\Throwable $e) {
            // Un cache non purgé ne doit pas faire échouer une réparation
            // réussie : au pire l'indicateur se corrige tout seul en 2 min.
        }
    }
