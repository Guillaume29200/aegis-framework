...
class MonitoringService
{
    public static function getAllInfo($db = null): array
    {
        ...
        $modules = self::getModulesInfo($db);
    ...
    {
        // Source unique de vérité : le changelog du framework, un fichier par
        // version, assemblé par ChangelogService.
        try {
            $v = (new \Framework\Services\ChangelogService())->current();
            if ($v !== '') { return 'v' . $v; }
        } catch (\Throwable $e) {}
        // Repli : ancien fichier de version (compat).
    ...
    /**
     * Taille des dossiers de l'application.
     *
     * C'est le relevé le plus coûteux de toute la page : il parcourt
     * l'arborescence entière et mettait à lui seul 2,3 des 2,4 secondes
     * d'affichage. Or une taille de dossier ne bouge pas d'une minute à
     * l'autre — on la mémorise.
     *
     * Le cache est volontairement court : assez pour que la page reste vive
     * en consultation, assez bref pour qu'un nettoyage de fichiers se voie
     * sans avoir à attendre.
     */
    public static function getFilesInfo(): array
    {
        $cle = 'system.monitoring.files';
        ...
        $calcul = self::computeFilesInfo();

        if (function_exists('cache_set')) {
            cache_set($cle, $calcul, 300);
        }
    ...
    /** Oublie la taille mémorisée — après un nettoyage, par exemple. */
    public static function forgetFilesInfo(): void
    {
        if (function_exists('cache_delete')) {
            try { cache_delete('system.monitoring.files'); } catch (\Throwable $e) {}
        }
    }

    private static function computeFilesInfo(): array
    {
        $rootPath = self::getAppRoot();
        // `__DIR__ . '/../logs'` désignait modules/System/logs — un dossier qui
        // n'existe pas. Le journal réel est framework/logs, comme l'annonçaient
        // déjà les libellés plus bas ; la page affichait donc 0 fichier et 0 o.
        $logsPath = $rootPath . '/framework/logs';
    ...
    {
        $logsPath = self::getAppRoot() . '/framework/logs';
        $logs = [];
        $totalSize = 0;
        $criticalCount = 0;
    ...
    /**
     * Tronquer le fichier d'erreurs de PHP.
     *
     * On tronque au lieu de supprimer : le descripteur que PHP garde ouvert
     * survivrait à l'effacement de l'entrée du répertoire, et les erreurs
     * suivantes iraient dans un fichier devenu invisible.
     */
    public static function purgePhpErrorLog(): array
    {
        $fichier = (string) ini_get('error_log');

        if ($fichier === '' || !is_file($fichier)) {
            return ['success' => false, 'message' => "Aucun fichier d'erreurs PHP n'est configuré."];
        }

        $avant = (int) @filesize($fichier);

        if (!is_writable($fichier)) {
            return ['success' => false, 'message' => 'Fichier non accessible en écriture : ' . $fichier];
        }

        if (@file_put_contents($fichier, '') === false) {
            return ['success' => false, 'message' => 'La purge a échoué.'];
        }

        self::forgetFilesInfo();

        return [
            'success' => true,
            'message' => sprintf('Fichier vidé (%s libérés).', self::formatBytes($avant)),
        ];
    }
    ...
    public static function getModulesInfo($db = null): array
    {
        $modulesPath = self::getAppRoot() . '/modules';
        $modules = [];

        // État d'activation réel : il est stocké en base (table `modules`),
        // PAS dans module.json. On construit une carte name(minuscule) => active.
        $activeMap = self::getModulesActiveMap($db);
        ...
        if (is_dir($modulesPath)) {
            foreach ((array)@scandir($modulesPath) as $dir) {
                ...
                // Cœur non désactivable (System, etc.) : toujours actif.
                $isCore = !empty($moduleData['core']) || !empty($moduleData['required']);
                $key = strtolower((string)($moduleData['name'] ?? $dir));

                if ($isCore) {
                    $enabled = true;
                } elseif (array_key_exists($key, $activeMap)) {
                    $enabled = $activeMap[$key];
                } else {
                    // Pas de ligne en base => module présent mais pas installé.
                    $enabled = false;
                }
                ...
                $modules[] = [
                    ...
                    'enabled' => $enabled,
                    'installed' => $isCore || array_key_exists($key, $activeMap),
                    'path' => $modulesPath . '/' . $dir,
                ];
            }
        }

        // Actifs d'abord, désactivés en dernier (au sein de chaque groupe, ordre
        // alphabétique déjà garanti par scandir() — usort est stable en PHP 8+).
        usort($modules, fn($a, $b) => ($b['enabled'] <=> $a['enabled']));

        return ['total_modules' => count($modules), 'modules' => $modules, 'modules_path' => $modulesPath];
    }

    /**
     * Carte name(minuscule) => bool(active) lue dans la table `modules`.
     * Renvoie [] si la base est indisponible (on retombe alors sur un défaut prudent).
     */
    private static function getModulesActiveMap($db): array
    {
        $pdo = self::resolvePdo($db);
        ...
        try {
            $rows = $pdo->query('SELECT name, active FROM modules')->fetchAll(\PDO::FETCH_ASSOC) ?: [];
        } catch (\Throwable) {
            return [];
        }
        $map = [];
        foreach ($rows as $row) {
            $map[strtolower((string)$row['name'])] = (bool)(int)$row['active'];
        }
        return $map;
    }
