...
<?php
// Jauges en direct, juste sous l'en-tête : c'est l'état de la machine
// maintenant, pas un relevé figé au chargement de la page.
...
<div class="ui-card mon-card">
    ...
    <div id="tab-logs" class="mon-pane">
        <div class="ui-grid cols-2">
            ...
            <article class="mon-panel" style="grid-column:1/-1"><h4>📁 Fichiers de logs</h4><div class="mon-panel-bar"><p class="u-muted"><strong id="logFilesCount"><?= count($logs['log_files'] ?? []) ?></strong> fichier(s), du plus volumineux au plus léger.</p><input type="search" id="logFilesSearch" class="form-control" placeholder="🔎 Filtrer par nom…" style="max-width:240px"></div><div class="mon-scroll"><table class="ui-table" id="logFilesTable"><thead><tr><th>Fichier</th><th>Taille</th><th>Modifié</th><th>Actions</th></tr></thead><tbody><?php foreach (($logs['log_files'] ?? []) as $log): ?><tr class="log-row" data-name="<?= htmlspecialchars(mb_strtolower((string)($log['name'] ?? '')), ENT_QUOTES) ?>"><td><?= mon_text($log['name'] ?? '') ?></td><td><?= mon_text($log['size'] ?? '') ?></td><td><?= mon_text($log['modified'] ?? '') ?></td><td class="u-nowrap"><button class="ui-btn sm primary" onclick="viewLog('<?= htmlspecialchars($log['name'] ?? '', ENT_QUOTES) ?>')">👁️ Voir</button> <button class="ui-btn sm danger" onclick="deleteLog('<?= htmlspecialchars($log['name'] ?? '', ENT_QUOTES) ?>')">🗑️</button></td></tr><?php endforeach; ?><tr id="logFilesEmpty" class="u-muted" <?= empty($logs['log_files']) ? '' : 'hidden' ?>><td colspan="4">Aucun fichier de log.</td></tr></tbody></table></div><div class="mon-pager" id="logFilesPager" hidden></div></article>
            <article class="mon-panel" style="grid-column:1/-1"><h4>🧾 Logs SQL récents</h4><div class="mon-panel-bar"><p class="u-muted">Limité aux 100 dernières entrées<?php if (($dbLogs['total'] ?? 0) > 0): ?> — <strong><?= mon_num($dbLogs['total']) ?></strong> au total en base<?php endif; ?>.</p><?php if (!empty($dbLogs['available']) && ($dbLogs['total'] ?? 0) > 0): ?><button type="button" class="ui-btn danger sm" onclick="purgeDbLogs()">🗑️ Tout vider (<?= mon_num($dbLogs['total']) ?>)</button><?php endif; ?></div><div class="mon-scroll"><table class="ui-table"><thead><tr><th>ID</th><th>Niveau</th><th>Message</th><th>URL</th><th>IP</th><th>Date</th><th></th></tr></thead><tbody><?php foreach (($dbLogs['recent'] ?? []) as $log): $lvl = $log['level'] ?? ''; $tone = in_array($lvl, ['ERROR','CRITICAL','SECURITY'], true) ? 'red' : ($lvl === 'WARNING' ? 'amber' : 'green'); ?><tr><td><?= mon_num($log['id'] ?? 0) ?></td><td><span class="ui-badge <?= $tone ?>"><?= mon_text($lvl) ?></span></td><td><?= mon_text($log['message'] ?? '') ?></td><td class="mon-path"><?= mon_text($log['method'] ?? '') ?> <?= mon_text($log['url'] ?? '') ?></td><td><?= mon_text($log['ip_address'] ?? '') ?></td><td><?= mon_text($log['created_at'] ?? '') ?></td><td><button class="ui-btn sm danger" onclick="deleteDbLog(<?= (int)($log['id'] ?? 0) ?>)">🗑️</button></td></tr><?php endforeach; ?><?php if (empty($dbLogs['recent'])): ?><tr><td colspan="7" class="u-muted">Aucun log SQL.</td></tr><?php endif; ?></tbody></table></div></article>
            <article class="mon-panel" style="grid-column:1/-1"><h4>🐞 Dernières erreurs PHP</h4><pre class="mon-pre"><?= mon_text(implode("\n", $logs['last_errors'] ?? [])) ?: 'Aucune erreur récente.' ?></pre><?php if (!empty($logs['last_errors'])): ?><button type="button" class="ui-btn danger sm u-mt" onclick="purgePhpLog()">🗑️ Vider le fichier d'erreurs PHP</button><?php endif; ?></article>
        </div>
    </div>

    <!-- MODULES -->
    <div id="tab-modules" class="mon-pane">
        <article class="mon-panel"><h4>🧩 Modules installés</h4><div class="mon-scroll"><table class="ui-table"><thead><tr><th>Module</th><th>Version</th><th>Statut</th><th>Chemin</th></tr></thead><tbody><?php foreach (($modules['modules'] ?? []) as $m): ?><tr><td><?= mon_text($m['name'] ?? '') ?></td><td><?= mon_text($m['version'] ?? '') ?></td><td><?= !empty($m['enabled']) ? '<span class="ui-badge green">✅ Actif</span>' : (!empty($m['installed']) ? '<span class="ui-badge amber">⏸️ Désactivé</span>' : '<span class="ui-badge gray">⬇️ Non installé</span>') ?></td><td class="mon-path"><?= mon_text($m['path'] ?? '') ?></td></tr><?php endforeach; ?><?php if (empty($modules['modules'])): ?><tr><td colspan="4" class="u-muted">Aucun module trouvé.</td></tr><?php endif; ?></tbody></table></div></article>
    </div>
</div>
...
/* Ligne d'en-tête d'un panneau : la légende à gauche, l'action à droite.
   Le bouton de purge vivait dans un autre panneau, loin de la table qu'il
   vide — on le cherchait sans le trouver et on supprimait ligne à ligne. */
.mon-panel-bar { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin: -6px 0 10px; }
.mon-panel-bar > p { margin: 0; }

/* Pagination : les fichiers sont déjà tous dans la page, on ne fait que
   masquer ceux des autres pages. Aucun aller-retour serveur, donc aucun
   rechargement — et le compteur ne peut pas mentir sur ce qui est affiché. */
.mon-pager { display: flex; align-items: center; justify-content: space-between; gap: 12px; flex-wrap: wrap; margin-top: 12px; font-size: 13px; }
.mon-pager .mon-pages { display: flex; gap: 4px; flex-wrap: wrap; }
.mon-pager button {
  min-width: 30px; padding: 4px 8px; cursor: pointer;
  border: 1px solid var(--line, #d7dcea); border-radius: 6px;
  background: var(--surface, #fff); color: var(--text, #1e293b);
  font: inherit; font-size: 12.5px;
}
.mon-pager button:hover:not(:disabled) { border-color: var(--accent, #6366f1); }
.mon-pager button.is-on { background: var(--accent, #6366f1); border-color: var(--accent, #6366f1); color: #fff; font-weight: 600; }
.mon-pager button:disabled { opacity: .4; cursor: default; }
...
/* Pagination des fichiers de logs, côté navigateur.
   Ils sont tous rendus dans la page : changer de page ne fait que masquer les
   lignes des autres, sans aucune requête ni rechargement. */
(function () {
    var PAR_PAGE = 10;

    var table  = document.getElementById('logFilesTable');
    var pager  = document.getElementById('logFilesPager');
    var champ  = document.getElementById('logFilesSearch');
    var vide   = document.getElementById('logFilesEmpty');
    var compte = document.getElementById('logFilesCount');
    if (!table || !pager) { return; }

    var lignes = [].slice.call(table.querySelectorAll('.log-row'));
    if (!lignes.length) { return; }

    var page = 1;

    function dessine(gardees) {
        var pages = Math.max(1, Math.ceil(gardees.length / PAR_PAGE));
        if (page > pages) { page = pages; }

        var debut = (page - 1) * PAR_PAGE;
        gardees.forEach(function (tr, i) { tr.hidden = (i < debut || i >= debut + PAR_PAGE); });

        if (vide) { vide.hidden = gardees.length > 0; }
        if (compte) { compte.textContent = gardees.length; }

        // Une seule page : la pagination n'apporte rien.
        if (pages <= 1) { pager.hidden = true; pager.innerHTML = ''; return; }

        var html = '<span class="u-muted">Page ' + page + ' sur ' + pages + '</span><span class="mon-pages">';
        html += '<button type="button" data-p="' + (page - 1) + '"' + (page === 1 ? ' disabled' : '') + '>←</button>';
        for (var p = 1; p <= pages; p++) {
            html += '<button type="button" data-p="' + p + '"' + (p === page ? ' class="is-on"' : '') + '>' + p + '</button>';
        }
        html += '<button type="button" data-p="' + (page + 1) + '"' + (page === pages ? ' disabled' : '') + '>→</button>';
        pager.innerHTML = html + '</span>';
        pager.hidden = false;
    }

    function applique(remetPage) {
        if (remetPage) { page = 1; }
        var mot = (champ && champ.value || '').trim().toLowerCase();

        var gardees = [];
        lignes.forEach(function (tr) {
            if (!mot || (tr.getAttribute('data-name') || '').indexOf(mot) !== -1) { gardees.push(tr); }
            else { tr.hidden = true; }
        });
    ...
    if (champ) { champ.addEventListener('input', function () { applique(true); }); }

    applique(true);
})();

window.purgePhpLog = function () {
    if (!confirm('Vider le fichier d\'erreurs de PHP ? Cette action est irréversible.')) return;
    post('<?= u('/admin/monitoring/purge-php-log') ?>', { confirmation: 'PURGE_PHP_LOG' }).then(d => { alert((d.success ? '✅ ' : '❌ ') + d.message); if (d.success) location.reload(); });
};
