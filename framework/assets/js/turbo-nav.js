...
(function () {
    ...
    const CONFIG = {
        ...
        cacheTTL:        45000,             // ms avant qu'une entrée de cache soit jugée périmée
                                            // (dashboards/compteurs en direct : sans TTL, une page
                                            // reste en cache — donc figée — jusqu'à éviction LRU)
    ...
    let   hoveredAnchor = null;   // lien <a> ayant démarré le timer en cours (voir handleMouseLeave)
    ...
    // ─── Cache (lecture/écriture avec TTL) ────────────────────────────────────
    // Une entrée périmée (au-delà de cacheTTL) est traitée comme absente : la
    // page est re-fetchée normalement. Évite d'afficher des chiffres figés
    // (compteurs, notifications en attente…) en revisitant une page mise en
    // cache il y a plusieurs minutes.
    ...
    function cachePut(url, html) {
        ...
        if (cache.size >= CONFIG.cacheMaxSize && !cache.has(url)) {
    ...
    async function prefetch(url) {
        // Déjà en cache (frais) ou déjà en cours de prefetch → inutile
        if (cacheGet(url) !== null || prefetching.has(url)) return;
    ...
    function handleMouseLeave(e) {
        // 'mouseleave' est écouté en phase de capture (voir init()) pour intercepter
        // les liens même sans bubbling — mais ça le fait se déclencher pour CHAQUE
        // élément enfant quitté (ex: passer de l'icône au texte dans <a>🏆 Texte</a>).
        // Sans ce filtre, le timer était annulé au moindre micro-mouvement de souris
        // à l'intérieur d'un lien à icône+texte, empêchant quasiment tout prefetch.
        // On n'annule que si c'est bien le lien qui a démarré le timer en cours.
        const anchor = e.target.closest('a');
    ...
    async function navigate(url, pushState = true) {
        ...
        try {
            ...
            // Vérifie le cache d'abord (entrées périmées ignorées, voir cacheGet)
            const cached = CONFIG.cacheEnabled ? cacheGet(url) : null;
            if (cached !== null) {
                html = cached;
            } else {
                const controller = new AbortController();
    ...
    function shouldIntercept(anchor) {
        ...
        // Laisse passer les liens avec data-no-turbonav (nom utilisé dans ce
        // codebase) ou data-no-turbo (nom documenté sur gamenodepanel.com —
        // alias accepté pour rester compatible avec la doc officielle).
        if (anchor.hasAttribute('data-no-turbonav') || anchor.hasAttribute('data-no-turbo')) return false;
    ...
    // Seuls les clics sur <a> étaient interceptés : les formulaires de filtre/
    // recherche (method="get", très courants — listes d'articles, de tournois…)
    // déclenchaient donc toujours un rechargement complet, cassant l'illusion
    // de navigation instantanée sur ces pages précisément.

    function shouldInterceptForm(form) {
        if (!form || form.tagName !== 'FORM') return false;

        // Seul GET a un sens à transformer en simple navigation d'URL ;
        // POST reste un rechargement classique (soumission de données).
        const method = (form.getAttribute('method') || 'get').toLowerCase();
        if (method !== 'get') return false;

        if (form.target && form.target !== '_self') return false;
        if (form.hasAttribute('data-no-turbonav') || form.hasAttribute('data-no-turbo')) return false;

        // Action externe (autre domaine) → laisser passer normalement
        const action = form.getAttribute('action') || '';
        if (/^[a-z][a-z0-9+.-]*:/i.test(action) && !action.startsWith(window.location.origin)) return false;
    ...
    function init() {
        ...
        // Écoute les soumissions de formulaires GET (filtres/recherche)
        ...
        console.info('[TurboNav] ✅ v2.1.0 actif — préchargement au survol activé');
    }
