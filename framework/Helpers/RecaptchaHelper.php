...
/**
 * Vérifier un token reCAPTCHA côté serveur.
 *
 * Renvoie true si la vérification réussit OU si reCAPTCHA est désactivé/non
 * configuré (bypass) — pour ne pas bloquer les formulaires quand la protection
 * n'est pas activée. Utiliser recaptcha_verify_details() pour le score complet.
 *
 * @param string $token  Jeton reçu (input caché recaptcha_token).
 * @param string $zone   Zone/action (login, register, contact…).
 * @param float  $minScore Score minimal accepté (v3).
 */
function recaptcha_verify(string $token, string $zone = 'submit', float $minScore = 0.5): bool
{
    $res = recaptcha_verify_details($token, $zone, $minScore);
    return !empty($res['success']);
}

/**
 * Vérification reCAPTCHA détaillée (tableau : success, score, bypass, error…).
 */
function recaptcha_verify_details(string $token, string $zone = 'submit', float $minScore = 0.5): array
{
    global $container;

    if (!isset($container)) {
        // Pas de conteneur : on ne bloque pas (bypass).
        return ['success' => true, 'bypass' => true, 'message' => 'reCAPTCHA indisponible'];
    }

    try {
        $recaptcha = $container->get('Framework\\Services\\RecaptchaService');
        return $recaptcha->verify($token, $zone, $minScore);
    } catch (\Exception $e) {
        return ['success' => true, 'bypass' => true, 'message' => 'reCAPTCHA indisponible'];
    }
}
