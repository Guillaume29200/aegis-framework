...
class AdminMenuService
{
    ...
    public function build(): array
    {
        ...
        // Tri par position, MAIS « Configuration » est TOUJOURS forcé en dernier,
        // quelles que soient les positions déclarées par les modules (règle produit).
        // usort est stable depuis PHP 8.0 → l'ordre par position est préservé à
        // l'intérieur de chaque groupe (config / non-config).
        usort($items, function ($a, $b) {
            $ac = self::isConfigurationItem($a) ? 1 : 0;
            $bc = self::isConfigurationItem($b) ? 1 : 0;
            if ($ac !== $bc) {
                return $ac <=> $bc; // 0 (non-config) avant 1 (config)
            }
    ...
    /**
     * L'item est-il l'entrée « Configuration » (à forcer en dernière position) ?
     * Repéré par son libellé OU son préfixe d'URL/match dédié.
     */
    private static function isConfigurationItem(array $item): bool
    {
        if (mb_strtolower(trim((string) ($item['label'] ?? ''))) === 'configuration') {
            return true;
        }
        $match = (string) ($item['match'] ?? $item['url'] ?? '');
        return $match === '/admin/configuration' || str_starts_with($match, '/admin/configuration/');
    }
