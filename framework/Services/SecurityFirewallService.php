...
class SecurityFirewallService
{
    ...
    private function detectSuspiciousRequest(string $path): ?string
    {
        ...
        foreach (($this->config['suspicious_paths'] ?? []) as $needle) {
            if ($needle !== '' && $this->pathMatchesProbe($lowerPath, strtolower((string)$needle))) {
    ...
    /**
     * Le chemin correspond-il à un sondage de scanner ?
     *
     * La comparaison était un simple str_contains, et c'était trop grossier :
     * « /backup » se retrouve dans « /admin/sauvegarde/backups », une route
     * parfaitement légitime. Un administrateur qui ouvrait la liste de ses
     * sauvegardes voyait donc son IP bannie une heure — et toutes les autres
     * pages avec, puisque le blocage porte sur l'IP.
     *
     * Ces motifs visent des sondages à la racine : /backup.zip, /.env,
     * /wp-login.php. On exige donc que le motif commence le chemin ET se
     * termine sur une frontière — fin, séparateur, extension ou requête.
     * « /backup » attrape ainsi /backup, /backup/, /backup.sql, mais laisse
     * passer /backups et /admin/sauvegarde/backups.
     */
    private function pathMatchesProbe(string $path, string $needle): bool
    {
