...
class ModuleManager
{
    ...
    /** Avertissement non bloquant de la dernière opération (ex: tables non supprimées). */
    ...
    public function loadModules(): void
    {
        ...
        // Câble les « capacités » transverses (Markdown, …) déclarées par les
        // modules actifs dans leur module.json. Sans effet si aucune capacité.
        try {
            \Framework\Capabilities\CapabilityManager::boot($this->modulesPath, $loadedNames);
        } catch (\Throwable $capErr) {
            $this->logger->warning('CapabilityManager boot failed', ['error' => $capErr->getMessage()]);
        }
    }
     ...
     */
    /**
     * @return bool true si le script a été trouvé et exécuté (ou était vide),
     *              false s'il était introuvable (ex: dossier du module déjà
     *              supprimé du disque) — l'appelant doit alors avertir plutôt
     *              que prétendre un nettoyage complet.
     */
    private function runModuleSqlFile(string $moduleName, string $file): bool
    {
        $path = $this->modulesPath . '/' . $moduleName . '/database/' . $file;
        if (!is_file($path)) {
            $this->logger->warning("Module SQL introuvable : {$moduleName}/database/{$file} — aucune table supprimée/créée.");
            return false;
        }
    ...
    /**
     * Oublie le compte mémorisé par l'indicateur de mises à jour de l'en-tête.
     *
     * Placé ICI plutôt que chez l'appelant : l'état d'un module change par
     * plusieurs chemins — le diagnostic, la page Modules, un script — et une
     * purge posée sur un seul d'entre eux laisse les autres afficher un
     * compte périmé. C'est exactement ce qui s'est produit : le diagnostic
     * annonçait deux mises à jour pendant que l'en-tête en montrait trois.
     */
    private function forgetUpdateCache(): void
    {
        if (function_exists('cache_delete')) {
            try { cache_delete('framework.module_updates'); } catch (\Throwable $e) {}
        }
    }
    ...
    /** Dernier avertissement non bloquant (ex: uninstall.sql introuvable → tables conservées). */
    ...
    public function activateModule(string $moduleName): bool
    {
        ...
        // Avant toute chose : le module a-t-il DÉJÀ des tables en base ? Si oui,
        // on ne devra JAMAIS lancer le nettoyage destructif (uninstall.sql = DROP)
        // en cas d'échec — ce serait une perte de données sur une simple ré-activation.
        $preexistingTables = false;
        try {
            $preexistingTables = $this->moduleHasAnyTable($moduleName);
        ...
        try {
            ...
            // Nettoyage : on ne supprime les tables (uninstall.sql = DROP) QUE s'il
            // s'agissait d'une PREMIÈRE installation (aucune table préexistante).
            // Sur une ré-activation d'un module déjà installé, on ne détruit JAMAIS
            // les données existantes — on se contente de le laisser inactif.
            if (!$preexistingTables) {
                ...
                $this->logger->warning("Activation échouée mais tables préexistantes conservées (pas de DROP) : {$moduleName}");
            }
    ...
    /**
     * Au moins une des tables déclarées par le module existe-t-elle déjà en base ?
     * Sert de garde-fou : on ne lance jamais le nettoyage destructif (DROP) sur un
     * module dont les tables — donc potentiellement les données — préexistent.
     */
    private function moduleHasAnyTable(string $moduleName): bool
    {
        $base = $this->modulesPath . '/' . $moduleName;
        $sql = '';
        if (is_file($base . '/database/install.sql')) {
            $sql = (string) file_get_contents($base . '/database/install.sql');
        } elseif (is_file($base . '/schema.sql')) {
        ...
        foreach (array_unique($m[1]) as $table) {
            if ($this->tableExists($table)) { return true; }
        }
        return false;
    }
    ...
    private function findMissingTables(string $moduleName): array
    {
        ...
        // Retire les commentaires SQL avant l'analyse : sinon un commentaire qui
        // mentionne « CREATE TABLE IF NOT EXISTS, … » est pris pour une table (« IF »).
        $sql = preg_replace('!/\*.*?\*/!s', '', $sql);            // blocs /* … */
        $sql = preg_replace('/^\s*(?:--|#).*$/m', '', $sql);      // lignes -- … et # …
    ...
    public function deactivateModule(string $moduleName): bool
    {
        ...
        try {
            ...
            if (!$this->runModuleSqlFile($moduleName, 'uninstall.sql')) {
                $this->lastWarning = "uninstall.sql introuvable pour {$moduleName} (dossier du module absent du disque ?) — ses tables n'ont PAS été supprimées et restent en base.";
            }
    ...
    public function deleteModule(string $moduleName): bool
    {
        ...
        try {
            ...
            if (!$this->runModuleSqlFile($moduleName, 'uninstall.sql')) {
                $this->lastWarning = "uninstall.sql introuvable pour {$moduleName} (dossier déjà absent du disque ?) — ses tables n'ont PAS été supprimées et restent en base. Un nettoyage manuel est nécessaire.";
            }
