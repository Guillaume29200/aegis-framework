...
class CSRFProtection
{
    ...
    public function __construct(array $config)
    {
        ...
        // Taille du pool pilotée par la config (défaut prudent si absente).
        $this->poolSize = max(10, (int)($this->config['pool_size'] ?? $this->poolSize));
    ...
    public function generateToken(string $action = 'default'): string
    {
        ...
        // Réutilise le dernier token encore valide plutôt que d'en créer un à
        // CHAQUE rendu de page. Sinon le pool se remplit très vite (navigation +
        // préchargement TurboNav qui rend les pages côté serveur) et évince les
        // tokens de formulaires encore ouverts → faux « session expirée ».
        // Un seul token par fenêtre de session = motif synchronizer-token,
        // sûr (aléatoire, expirable, lié à la session) et sans éviction.
        if (!empty($_SESSION['csrf_tokens'][$action])) {
            $last = end($_SESSION['csrf_tokens'][$action]);
            if (is_array($last) && isset($last['expire'], $last['token']) && $last['expire'] > time()) {
                return $last['token'];
            }
        }
