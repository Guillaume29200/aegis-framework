...
class XSSProtection
{
     ...
     *
     * Désactivé par défaut (voir config/security.php, section 'xss') : cette
     * méthode appelle cleanArray() -> clean() -> strip_tags() SANS liste
     * blanche sur toute valeur, ce qui mutile silencieusement les données
     * légitimes contenant '<'/'>' (JSON, code, HTML d'éditeur riche...). La
     * vraie protection XSS se fait à l'affichage via escape()/escapeAttr()/
     * escapeJs()/escapeUrl(), pas en modifiant l'entrée avant que le
     * contrôleur ne la reçoive.
     */
